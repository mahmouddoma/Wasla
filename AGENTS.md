...
16. **Strictly NO colored borders on containers/cards (use neutral subtle borders only).**
17. **NO card-overuse / carditis (use unified lists and seamless surfaces instead of nesting cards).**
18. **Slide-over sidebars/drawers for reviews and detail inspection (not separate full pages).**
...
# 32. WASLA UI/UX & DESIGN SYSTEM STANDARDS (STRICT USER PREFERENCES)

These rules address recurring user feedback and enforce strict aesthetic and functional standards across Wasla:

### 32.1 Strictly NO Colored Borders (ممنوع البوردرات الملونة)

- **NEVER** use colored or saturated borders (such as teal, cyan, blue, or red borders) around cards, panels, containers, or inputs by default.
- Default borders must ALWAYS be neutral and subtle: `var(--wasla-border-subtle)`, `var(--wasla-border)` (`#e2e8f0` / `#f1f5f9`).
- Primary/accent colors (e.g., Wasla Teal) are strictly reserved for:
  - Subtle focus rings (`box-shadow: 0 0 0 3px rgba(11, 165, 160, 0.12)`)
  - Active tab indicators
  - Status badges and pills
  - Primary action buttons
  - Small icon accents / dots

### 32.2 Eliminate "Carditis" — Do NOT Put Everything in a Card (مش كل حاجة تتحط في Card)

- **Stop wrapping every small item, form section, list item, or toggle in isolated cards with heavy backgrounds, borders, and drop-shadows.**
- Integrate form sections directly onto the main panel surface with clean spacing and subtle dividers (`border-bottom: 1px solid var(--wasla-border-subtle)`).
- For listings (e.g., emergency contacts, patient requests, doctor schedules), use a unified directory list or structured table (`.contacts-list-table`) instead of repeated detached card blocks.
- Toggles and switches must be clean inline rows (`.primary-toggle-row`), not bulky card containers.

### 32.3 Slide-Over Drawers / Sidebars for Details & Reviews (سايدبار مش صفحة تانية)

- When a user clicks to view request details, review an application, inspect an item, or perform quick edits:
  - **Do NOT navigate away to a separate full page** unless explicitly asked.
  - Implement a sleek **Slide-over Drawer / Sidebar** overlay that slides in from the edge (e.g., right edge in RTL).
  - This preserves the user's working context and table scroll position.

### 32.4 Mandatory Toast Notifications on All Mutations (التوستر شغال دايماً مع التعديل والحفظ)

- Every user action that saves, updates, edits, adds, or deletes data **MUST** trigger a clear toast notification using `ToastService`:
  - `this.toastService.success('تم الحفظ بنجاح')`
  - `this.toastService.error('فشلت العملية...')`
- Never perform silent data mutations without visible, reassuring feedback.

### 32.5 Form Controls & Select Dropdowns Completeness (تناسق الحقول ومظهر الـ Select)

- **Never leave native browser `<select>` elements unstyled.**
  - Always apply `appearance: none; -webkit-appearance: none; -moz-appearance: none;` to eliminate ugly default browser arrows and black borders.
  - Match `<select>` exactly with `.field-input`: `height: 42px`, `background: #fcfdfe`, `border: 1px solid var(--wasla-border)`, `border-radius: var(--radius-control, 10px)`.
  - Position custom SVG arrows using CSS logical properties (`inset-inline-end: 14px`) and ensure adequate padding (`padding-inline-end: 38px`) so text never overlaps the icon in RTL or LTR.
- **Custom Switches & Checkboxes**:
  - Native checkboxes must be hidden completely using `.sr-only-chk` (`opacity: 0`, `position: absolute`, etc.), never leaving raw browser checkboxes visible next to custom toggles.
- **Numeric & Phone Inputs**:
  - Always apply `dir="ltr"` and `.ltr-text` for phone numbers, national IDs, and timestamps to ensure proper bidirectional display.

### 32.6 Bilingual Completeness — Arabic & English Mandatory (التأكد دايماً من توفر الترجمة عربي وإنجليزي)

- **Every UI string MUST have both Arabic (`ar`) and English (`en`) translations.**
  - Centralized in `src/app/core/i18n/translations.ts` under the `TRANSLATIONS` dictionary.
  - Whenever adding new features, sections, buttons, tooltips, dialogs, validation errors, or toast messages:
    1. Register the translation key in `TRANSLATIONS` with both `{ ar: '...', en: '...' }`.
    2. Consume in templates via `TranslatePipe`: `{{ 'feature.key' | translate }}`.
    3. Consume in TypeScript / services via `LanguageService` or `TRANSLATIONS[key]`.
