...
import { DoctorPracticeResponse } from '../../../../domains/doctor-practices';
...
describe('DoctorPractices', () => {
  ...
  it('creates component and loads practice list with computed active counts', async () => {
    ...
    expect(component['practices']().length).toBe(2);
  ...
  it('supports reload to re-fetch practices and reset error states', async () => {
    const req = httpTesting.expectOne(`${environment.apiBaseUrl}/api/v1/doctors/me/practices`);
    req.flush(samplePractices);
    await fixture.whenStable();

    component['isNotFound'].set(true);
    component['isForbidden'].set(true);

    const reloadPromise = component['reload']();
    const reloadReq = httpTesting.expectOne(`${environment.apiBaseUrl}/api/v1/doctors/me/practices`);
    reloadReq.flush(samplePractices);
    await reloadPromise;

    expect(component['isNotFound']()).toBe(false);
    expect(component['isForbidden']()).toBe(false);
  });
});
