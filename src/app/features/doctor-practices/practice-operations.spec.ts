...
describe('PracticeOperations', () => {
  ...
  it('creates component and loads configuration and branding in parallel', async () => {
    ...
    expect((component as any).branding()?.primaryColor).toBe('#008C8C');
    expect((component as any).configuration()?.defaultSlotDurationMinutes).toBe(20);
  ...
  it('updates color picker value and branding model when onColorPicked is triggered', async () => {
    ...
    (component as any).onColorPicked('primaryColor', { target: mockInput } as unknown as Event);

    expect((component as any).brandingModel().primaryColor).toBe('#099268');
    expect((component as any).getValidHex('#099268', '#008C8C')).toBe('#099268');
    expect((component as any).getValidHex('invalid', '#008C8C')).toBe('#008C8C');
  });
});
